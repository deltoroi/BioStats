...
library (tidyverse)
library (car)
sperm<- read_csv("~/Desktop/Bio170Labs/Lab05- t-test/WormSperm.csv")
bbirds<- read_csv("~/Desktop/Bio170Labs/Lab05- t-test/BlackbirdTesto.csv")
```
...
ggplot (bbirds, aes(Antibody.production, fill=Treatment)) + geom_density(alpha=0.5) + xlim(0,200)
ggplot (bbirds, aes (sample=Antibody.production, color=Treatment)) + geom_qq() + stat_qq_line()
...
```{r, echo=FALSE}
?t.test
t.test (Antibody.production ~ Treatment, data=bbirds, var.equal=TRUE)
...
ggplot (bbirds, aes(x=Treatment, y=Antibody.log)) + 
  geom_point(size=6, levels = c("Before", "After"), color="darkgrey") +
